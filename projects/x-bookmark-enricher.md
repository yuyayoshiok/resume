...
- **テーマ別まとめ**: 指定したテーマで関連ブックマークを集め、共通して語られていることを統合した記事を生成
- **ダッシュボード**: 処理済み件数、カテゴリ分布、記事ネタ候補の上位20件、興味の傾向を表形式で自動生成
...
- **Obsidianに馴染む作り**: frontmatterを正しく生成し、プラグインなしでも見られる表形式のダッシュボードを出力。既存ノートへのリンクも張ります
