...
業務プロセスの見直しと自動化ツールの企画・開発を一貫して担当。生成AIの業務活用をいち早く推進し、現場の工数削減を実現。
...
- 生成AI（ChatGPT・Gemini）の社内導入支援および活用レクチャーを主導
- GitHub Copilotを用いたシステム開発フローの構築・整備
- VoC（顧客の声）分析ツールをゼロから設計・実装し、分析工数を削減
...
吃音というハンディキャップを、正確な事務処理能力とITスキルによる業務効率化で補い、エンジニアとしてのキャリアを確立。「自分が楽になるために自動化する」という動機から始まったプログラミングが、今では組織全体の業務改善に直結しています。

**最新技術への適応力**

生成AI登場時にいち早く業務活用を探求。未経験からPython・GASを習得し、G検定・AWS資格を取得。個人プロダクトで常に新技術を実運用で試し、現場に持ち込む習慣があります。
