...
**Python × データ処理・業務自動化エンジニア**

システム開発経験3年以上。BPO企業で月20万件の顧客データ（VoC）をAzure OpenAIで自動集計・分析する仕組みを構築し、約200人月かかっていた作業を1-2名運用まで圧縮。
個人開発では複数のLLM（Codex / Gemini / Groq）を役割分担させたAIアシスタント基盤を設計・本番運用中です。

「自分が直面する課題をテクノロジーで解決する」を原動力に、データ集計・自動化のバックエンドから、AWS/GCP/AzureのAI連携、Terraformによるインフラのコード管理、フロントエンド（React/Next.js）までを一人で一貫して構築しています。
...
- Slackをインターフェースに、日次レビュー・RSS要約・ヘルスデータ監視・コスト集計などのデータ収集・集計を自動化
...
GAS/VBAによる業務自動化は、個別業務に最適化した改善の積み重ねでした。ツールを作るたびに、「そもそもシステムとして設計されていれば、この手戻りは要らなかったはずだ」と感じる場面が積み重なりました。

並行して、個人開発でバックエンド・インフラ・フロントエンドまで一人で構築する経験を重ねる中で、業務改善ツールの枠を超え、要件定義から設計・実装・運用まで一貫して担うWebアプリケーション開発に挑戦したいという意欲が育ちました。

現場で手を動かしてきたからこそ、利用者視点で課題を捉えられるのが自分の強みです。受け身で作業をこなすのではなく、設計・実装に主体的に責任を持つ立場で、クライアントの事業課題を解くものづくりに関わりたいと考えています。
...
個人開発は一人でも、実務ではチーム内の非エンジニアも含めた業務改善提案・ヒアリングを重ねながらツールを作ってきました。

GitHubでのブランチ運用・Pull Request・コミット履歴による変更管理を個人プロダクトでも徹底しており、複数人での開発フローにもすぐ馴染める体制を意識しています。
...
|---|---|
| 雇用形態 | 正社員 |
| 働き方 | 出社・ハイブリッド・リモートいずれも可 |
| 勤務地 | 島根県松江市またはリモート |
| 得意領域 | Python × データ処理・業務自動化 / Webアプリケーション開発 |
| 興味ある案件 | Webアプリケーション開発（バックエンド・フロントエンド） / データパイプライン構築 / サーバーレスアーキテクチャ |
